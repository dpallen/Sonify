...
public class PhraseView extends JPanel {
    ...
    public PhraseView(Phrase phrase) {
        ...
        x1Label = new JLabel("x1: " + String.valueOf(formatter.format(phrase.getX1())));
        this.add(x1Label);
        x2Label = new JLabel("x2: " + String.valueOf(formatter.format(phrase.getX2())));
        this.add(x2Label);
        setBorder(BorderFactory.createLineBorder(phrase.getCompound().getBorderColor(), 3, true));
    }

    public void updatePanel() {
        if (phrase.isSelected()) {
            setBackground(phrase.getCompound().getSelectedColor());
        }
        else setBackground(phrase.getCompound().getUnselectedColor());
        setBorder(BorderFactory.createLineBorder(phrase.getCompound().getBorderColor(), 3, true));
    }
